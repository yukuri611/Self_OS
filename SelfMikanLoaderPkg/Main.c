...
EFI_STATUS SaveMemoryMap(struct MemoryMap* map, EFI_FILE_PROTOCOL* file) {
    ...
    if (EFI_ERROR(status)) {
        Print(L"failed to write header: %r\n", status);
        return status;
    }
    ...
    for (iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0;
        ...
        if (EFI_ERROR(status)) {
            Print(L"failed to write memory map: %r\n", status);
            return status;
        }
    }
...
EFI_STATUS OpenGOP(EFI_HANDLE image_handle, 
    ...
    if (EFI_ERROR(status)) {
        Print(L"failed to locate handle buffer: %r\n", status);
        return status;
    }
...
EFI_STATUS EFIAPI UefiMain(
    ...
    if (EFI_ERROR(status)) {
        ...
        if (EFI_ERROR(status)) {
            Print(L"SaveMemoryMap failed: %r\n", status);
            Halt();
        }

...
void LayerManager::UpDown(unsigned int id, int new_height) {
    if(new_height < 0) {
        //delete from the stack(hide the layer)
        Hide(id);
        return;
    }
    if (new_height > layer_stack_.size()) {
        // limit the height to the current stack size.
    ...
    if (old_pos == layer_stack_.end()) { //if the layer is currently hided
        layer_stack_.insert(new_pos, layer);
        return;
    }

    if (new_pos == layer_stack_.end()) { // we are going to delete the layer first so layer_stack_.end() will become 1 smaller
        --new_pos;
    }

...
namespace {
    ...
    Error ScanFunction(uint8_t bus, uint8_t device, uint8_t function) {
        ...
        if (class_code.Match(0x06u, 0x04u)) { // PCI bridge
...
namespace pci {
    ...
    Error ScanAllBus() {
        ...
        auto header_type = ReadHeaderType(0, 0, 0); // Read header type of host bridge
        if (IsSingleFunctionDevice(header_type)) { //host bridge is only for bus 0
            return ScanBus(0);
        }

        for (uint8_t function = 1; function < 8; ++function) { // Scan all bus conncted to host bridge

...
void SetDataSegment(SegmentDescriptor& desc,
    ...
    desc.bits.default_operation_size = 1; // 1: 32bit
}

void SetupSegments() {
    gdt[0].data = 0; // null descriptor
    SetCodeSegment(gdt[1], DescriptorType::kExecuteRead, 0, 0, 0xfffff); // kernel code segment
    SetDataSegment(gdt[2], DescriptorType::kReadWrite, 0, 0, 0xfffff); // kernel data segment

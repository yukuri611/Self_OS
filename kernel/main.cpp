...
extern "C" void KernelMainNewStack(
    ...
    for (int i = 0; i < pci::num_device; ++i) {
        if (pci::devices[i].class_code.Match(0x0cu, 0x03u, 0x30u)) {
            ...
            if(0x8086 == pci::ReadVendorId(*xhc_dev)) { //if vendor is Intel
                break;
            }
        }
    }
    ...
    const uint8_t bsp_local_apic_id = 
        *reinterpret_cast<uint8_t*>(0xfee00020) >> 24;
    ...
    // Read xHC MMIO address
    const WithError<uint64_t> xhc_bar = pci::ReadBar(*xhc_dev, 0); // read BAR0 which is storing MMIO address
    ...
    WriteString(*main_window->Writer(), {24,28}, "Welcome from", {0, 0, 0});
    WriteString(*main_window->Writer(), {24,44}, "yukuri611!", {0, 0, 0});
            ...
            case Message::kInterruptXHCI:
                while (xhc.PrimaryEventRing()->HasFront()) {
                    if (auto err = ProcessEvent(xhc)) {
                        Log(kError, "ProcessEvent: %s at %s:%d\n",
                            err.Name(), err.File(), err.Line());
                    }
                }
                break;
